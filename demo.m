...
addpath('Data\');
addpath('superpixel\');
addpath('kernel_estimation\');

S=imread('data\original_rosis.tif');
% S = S(1:100,1:100,:);%% for estimation
F=load('data\R.mat');
...
REF = S;
%% fusion
Out = mymodel_FDA(HSI,MSI,F,par.fft_B);
Out_RGB = cat(3,Out(:,:,60),Out(:,:,29),Out(:,:,7));%PU
